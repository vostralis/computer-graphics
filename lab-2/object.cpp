...
void Object::loadObject(const QString& path) {
    ...
    while (!in.atEnd()) {
        ...
        if (parts[0] == 'v' && parts.size() == 4) {
            ...
            // Faces
            if (parts.size() < 4) {
                throw std::runtime_error("Face should at least consist of 3 points.");
            }

            QVector<int32_t> face;
            for (size_t i = 1; i < parts.size(); ++i) {
                face.emplaceBack(parts[i].toInt());
            }
...
const QVector<QVector<int32_t>>& Object::getFaces() const {
    return m_faces;
}
