...
public:
    ...
    // Enumeration that is responsible for object's render mode
...
private:
    ...
    // Draw arrows on the tip of axis
    void drawArrow(QPainter &painter, QPointF start, QPointF end, const QColor &color);
    // Draw axis X, Y and Z
    ...
    // Determine if current face is visible
    bool isFaceVisible(const QVector<int32_t>& face, const QMatrix4x4& modelViewMatrix) const;
    // Construct polygons for the painter
    void buildFaces(QVector<QVector<QPointF>>& visiblePolygons, QVector<QVector<QPointF>>& invisiblePolygons);
    // Render object
    void renderFaces(QPainter& painter);
